...
// TODO 处理读取文件错误
// 读取web应用配置数据
var appconfigPath = path.join(__dirname, 'conf/appconfig.' + process.env.NODE_ENV.trim() + '.json');
var appconfig = JSON.parse(fs.readFileSync(appconfigPath));

// 设置静态资源目录
if (appconfig.ENV == 'local') {
    appconfig.webresourcePackageName = 'v_local';
} else {
...
app.use(favicon(path.join(__dirname, './web/Public/static', 'favicon.ico')));
